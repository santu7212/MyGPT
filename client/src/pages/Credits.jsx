...
const Credits = () => {
  ...
  const fetchPlans = async () => {
    try {
      const { data } = await axios.get("/api/v1/credit/get-plan", {
          ...
          >
            <div className='flex-1'>
              ...
              <p className='text-2xl font-bold text-purple-600 dark:text-purple-300 mb-4'>
                ${plan.price}
                <span className='text-base font-normal text-grey-600 dark:text-purple-200'>
                  {" / "}
                  {plan.credits} credits
                </span>
              </p>
              <ul className='list-disc list-inside text-sm text-grey-700 dark:text-purple-200 space-y-1'>
